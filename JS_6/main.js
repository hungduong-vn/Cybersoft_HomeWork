...
document.getElementById("run5").onclick = function () {
  var n = document.getElementById("input5_1").value * 1;
  ...
  if (n < 2) {
    ...
    for (var i = 2; i <= n; i++) {
      ...
      var isPrime = true; //Not prime number
      for (var j = 2; j < i; j++) {
        if (i % j === 0) {
          isPrime = false;
          break;
  ...
  document.getElementById("output5_1").innerHTML = result;
  document.getElementById("output5_1").classList.add("showResult");
};
